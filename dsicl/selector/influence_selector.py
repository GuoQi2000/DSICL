...
class InfluenceSelector(BaseSelector):
    ...
    def select(self, data: DataReader, shots: int, subset_num = 1000, val_data = 200) -> DataReader:
        train_data = data
        train_data = train_data.split(num=200)[0]
        subsets = self.subset_collection(train_data, subset_num, shots)
        ...
        best_idxs = torch.topk(influences, shots).indices
