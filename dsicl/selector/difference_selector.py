...
class DifferenceSelector(BaseSelector):
    def __init__(self, model, tokenizer, prompter: Prompter, labels, candidate_num) -> None:
    ...
    def select(self, data: DataReader, shots: int, val_data:DataReader = None) -> DataReader:
        candidates = data
        differences = []
        scores = []
